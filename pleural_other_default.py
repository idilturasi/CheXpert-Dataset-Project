#Pleural Other trial 1: Everything kept the same without blur
...
def main():
    ...
    # Create dataset
    pathology = 'Pleural Other' # Replace with pathology
    ...
    torch.save(model.state_dict(), 'model3_pleural_other1_weights.pth')

    print("saved weights to file: model3_pleural_other1_weights.pth")
    ...
    # Load the CSV
    submission_df = pd.read_csv("model2_submission_only_improved.csv") 
    submission_df[pathology] = [(value * 2) - 1 for value in output_arr]
    submission_df.to_csv("pleural_other_default.csv", index=False)
